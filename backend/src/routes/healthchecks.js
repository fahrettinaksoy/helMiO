...
healthChecksRouter.post(
  ...
  ah(async (req, res) => {
    ...
    audit(req, {
      ...
      target: check.target,
      detail: `${check.type} · ${check.action}`,
    });
    res.status(201).json(check);
  }),
);

healthChecksRouter.put(
  '/:id',
  canManage,
  ah(async (req, res) => {
    const existing = await healthCheckStore.get(req.params.id);
    if (!existing || existing.serverId !== req.server.id)
      return res.status(404).json({ error: 'Kontrol bulunamadı' });
    const parsed = healthCheckSchema.safeParse(req.body);
    if (!parsed.success) {
      return res.status(400).json({ error: 'Doğrulama hatası', details: parsed.error.flatten() });
    }
    const check = await healthCheckStore.update(req.params.id, {
      ...parsed.data,
      serverId: req.server.id,
    });
    await healthCheckScheduler.reload();
...
healthChecksRouter.post(
  ...
  ah(async (req, res) => {
    ...
    if (!check || check.serverId !== req.server.id)
      return res.status(404).json({ error: 'Kontrol bulunamadı' });

...
serversRouter.post('/test', ah(async (req, res) => {
  const data = { ...req.body };
  if (data.id) {
  ...
  if (!parsed.success) {
    return res.status(400).json({ ok: false, error: 'Doğrulama hatası', details: parsed.error.flatten() });
  }
  const result = await installerService.testConnection({ ...parsed.data, id: data.id || 'adhoc', updatedAt: 'adhoc' });
  res.json(result);
}));

// List all servers (secrets masked).
serversRouter.get('/', ah(async (req, res) => {
  const servers = await serverStore.list();
  res.json(servers.map(publicServer));
}));

// Create a server.
serversRouter.post('/', requirePermission(PERMISSIONS.SERVER_MANAGE), ah(async (req, res) => {
  ...
  audit(req, { action: 'server.create', serverId: server.id, target: server.name, detail: `yöntem: ${server.method}` });
  res.status(201).json(publicServer(server));
}));

// Get one server.
serversRouter.get('/:id', ah(async (req, res) => {
...
// Update a server.
serversRouter.put('/:id', requirePermission(PERMISSIONS.SERVER_MANAGE), ah(async (req, res) => {
...
// Delete a server.
serversRouter.delete('/:id', requirePermission(PERMISSIONS.SERVER_MANAGE), ah(async (req, res) => {
  const existing = await serverStore.get(req.params.id);
...
serversRouter.post('/:id/test', ah(async (req, res) => {
  const server = await serverStore.get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Sunucu bulunamadı' });
  try {
...
serversRouter.get('/:id/diagnose', ah(async (req, res) => {
  const server = await serverStore.get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Sunucu bulunamadı' });
  try {
...
serversRouter.get('/:id/daemon', ah(async (req, res) => {
  const server = await serverStore.get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Sunucu bulunamadı' });
  try {
...
serversRouter.post('/:id/daemon/reload', requirePermission(PERMISSIONS.DAEMON_RELOAD), ah(async (req, res) => {
  const server = await serverStore.get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Sunucu bulunamadı' });
  try {
    const result = await supervisorService.reloadConfig(server);
    audit(req, { action: 'daemon.reload', serverId: server.id, target: server.name,
      detail: `+${result.added?.length || 0} ~${result.changed?.length || 0} -${result.removed?.length || 0}` });
    res.json({ ok: true, ...result });
  } catch (err) {
    audit(req, { action: 'daemon.reload', serverId: server.id, target: server.name, status: 'error', detail: err.message });
    res.status(502).json({ ok: false, error: err.message });
  }
}));

serversRouter.post('/:id/daemon/restart', requirePermission(PERMISSIONS.DAEMON_RESTART), ah(async (req, res) => {
  const server = await serverStore.get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Sunucu bulunamadı' });
  try {
    ...
    audit(req, { action: 'daemon.restart', serverId: server.id, target: server.name, status: 'error', detail: err.message });
    res.status(502).json({ ok: false, error: err.message });
  }
}));

serversRouter.post('/:id/daemon/log/clear', requirePermission(PERMISSIONS.PROCESS_CONTROL), ah(async (req, res) => {
  const server = await serverStore.get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Sunucu bulunamadı' });
  try {
...
serversRouter.post('/:id/daemon/shutdown', requirePermission(PERMISSIONS.DAEMON_RESTART), ah(async (req, res) => {
  const server = await serverStore.get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Sunucu bulunamadı' });
  try {
    ...
    audit(req, { action: 'daemon.shutdown', serverId: server.id, target: server.name, status: 'error', detail: err.message });
    res.status(502).json({ ok: false, error: err.message });
  }
}));
...
serversRouter.get('/:id/config', ah(async (req, res) => {
  const server = await serverStore.get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Sunucu bulunamadı' });
  try {
...
serversRouter.get('/:id/config/file', ah(async (req, res) => {
  const server = await serverStore.get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Sunucu bulunamadı' });
  try {
...
serversRouter.put('/:id/config/file', requirePermission(PERMISSIONS.CONFIG_WRITE), ah(async (req, res) => {
  const server = await serverStore.get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Sunucu bulunamadı' });
  try {
    ...
    audit(req, { action: 'config.write', serverId: server.id, target: req.body?.path, status: 'error', detail: err.message });
    res.status(502).json({ ok: false, error: err.message });
  }
}));

serversRouter.post('/:id/config/program', requirePermission(PERMISSIONS.CONFIG_WRITE), ah(async (req, res) => {
  const server = await serverStore.get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Sunucu bulunamadı' });
  try {
    ...
    audit(req, { action: 'config.add_program', serverId: server.id, target: req.body?.name, status: 'error', detail: err.message });
    res.status(502).json({ ok: false, error: err.message });
  }
}));

// Preview the [program:…] block for a definition without writing it.
serversRouter.post('/:id/config/program/preview', requirePermission(PERMISSIONS.CONFIG_WRITE), ah(async (req, res) => {
  try {
...
serversRouter.get('/:id/config/program/parse', requirePermission(PERMISSIONS.CONFIG_WRITE), ah(async (req, res) => {
  const server = await serverStore.get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Sunucu bulunamadı' });
  try {
    const content = await supervisorService.readConfigFile(server, req.query.path);
...
serversRouter.get('/:id/host', ah(async (req, res) => {
  const server = await serverStore.get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Sunucu bulunamadı' });
  try {
...
serversRouter.get('/:id/eventlistener', requirePermission(PERMISSIONS.CONFIG_WRITE), ah(async (req, res) => {
  const server = await serverStore.get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Sunucu bulunamadı' });
  try {
...
serversRouter.post('/:id/eventlistener/install', requirePermission(PERMISSIONS.CONFIG_WRITE), ah(async (req, res) => {
  const server = await serverStore.get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Sunucu bulunamadı' });
  try {
    ...
    audit(req, { action: 'eventlistener.install', serverId: server.id, target: server.name, status: 'error', detail: err.message });
    res.status(502).json({ ok: false, error: err.message });
  }
}));

serversRouter.post('/:id/eventlistener/uninstall', requirePermission(PERMISSIONS.CONFIG_WRITE), ah(async (req, res) => {
  const server = await serverStore.get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Sunucu bulunamadı' });
  try {
...
serversRouter.post('/:id/eventlistener/rotate-token', requirePermission(PERMISSIONS.CONFIG_WRITE), ah(async (req, res) => {
  const server = await serverStore.get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Sunucu bulunamadı' });
...
serversRouter.get('/:id/events', ah(async (req, res) => {
  const server = await serverStore.get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Sunucu bulunamadı' });
...
serversRouter.get('/:id/metrics', ah(async (req, res) => {
  const server = await serverStore.get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Sunucu bulunamadı' });
...
serversRouter.get('/:id/snapshot', ah(async (req, res) => {
  const server = await serverStore.get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Sunucu bulunamadı' });
  try {

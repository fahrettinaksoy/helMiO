...
channelsRouter.put('/:id', ah(async (req, res) => {
...
channelsRouter.post('/:id/test', ah(async (req, res) => {
  ...
  try {
    ...
    audit(req, { action: 'channel.test', target: channel.name, status: 'error', detail: err.message });

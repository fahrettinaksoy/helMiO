...
processesRouter.get(
  ...
  ah(async (req, res) => {
    ...
    try {
      const result = await supervisorService.readProcessLog(
        req.server,
        req.params.fullName,
        channel,
        offset,
...
processesRouter.get(
  '/:fullName/log/download',
  ah(async (req, res) => {
    const channel = req.query.channel === 'stderr' ? 'stderr' : 'stdout';
    try {
      const { data, truncated } = await supervisorService.downloadProcessLog(
        req.server,
        req.params.fullName,
        channel,
      );
...
groupsRouter.post(
  ...
  ah(async (req, res) => {
    try {
      const result = await supervisorService.startGroup(
        req.server,
...
groupsRouter.post(
  ...
  ah(async (req, res) => {
    try {
      const result = await supervisorService.restartGroup(
        req.server,
        req.params.groupName,
        wait(req),
      );
...
bulkRouter.post(
  ...
  ah(async (req, res) => {
    ...
    try {
      ...
      audit(req, {
        action: 'bulk.signal_all',
        serverId: req.server.id,
        target: req.server.name,
        detail: `SIG${sig}`,
      });
      res.json({ ok: true });
    } catch (err) {

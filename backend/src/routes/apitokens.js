...
apiTokensRouter.get('/', ah(async (req, res) => {
  res.json(await apiTokenStore.list());
}));

apiTokensRouter.post('/', ah(async (req, res) => {
  const parsed = createSchema.safeParse(req.body);
  if (!parsed.success) {
    return res.status(400).json({ error: 'Doğrulama hatası', details: parsed.error.flatten() });
  }
  const { token, record } = await apiTokenStore.create({ ...parsed.data, createdBy: req.user.username });
...
apiTokensRouter.delete('/:id', ah(async (req, res) => {

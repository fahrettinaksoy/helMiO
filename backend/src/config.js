...
export const config = {
  ...
  alertPollIntervalMs:
    process.env.ALERT_POLL_INTERVAL_MS != null ? Number(process.env.ALERT_POLL_INTERVAL_MS) : 30000,
  ...
  publicUrl: (
    process.env.HELMIO_PUBLIC_URL || `http://localhost:${Number(process.env.PORT) || 3001}`
  ).replace(/\/$/, ''),
};

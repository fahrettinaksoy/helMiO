...
const methodMeta = computed(() => METHOD_META[server.value?.method] || { icon: 'mdi-server', color: 'primary', label: server.value?.method || '' });
...
const daemonStyle = computed(() => (snapshot.value?.state ? stateStyle(snapshot.value.state.statename) : null));

const stats = computed(() => {
  const s = snapshot.value?.summary;
  if (!s) return [];
  return [
    { label: t('common.total'), value: s.total, color: 'primary', icon: 'mdi-format-list-bulleted' },
...
function subscribe() { if (serverId.value) realtime.subscribe(serverId.value); }
function unsubscribe() { if (serverId.value) realtime.unsubscribe(serverId.value); }

// Daemon info (supervisord version / pid)
const daemon = ref(null);
async function loadDaemon() {
  try { daemon.value = await serversApi.daemon(serverId.value); } catch { daemon.value = null; }
}

// Host metrics (shell connectors only)
const host = ref(null);
async function loadHost() {
  try { host.value = await serversApi.host(serverId.value); } catch { host.value = null; }
}
let hostTimer = null;
const memPct = computed(() => (host.value?.mem ? Math.round((host.value.mem.usedMb / host.value.mem.totalMb) * 100) : 0));
const loadHigh = computed(() => host.value?.load && host.value?.cores && host.value.load.one > host.value.cores);
...
const RANGES = [{ v: 15, label: '15d' }, { v: 60, label: '1s' }, { v: 360, label: '6s' }, { v: 1440, label: '24s' }];
...
const procSeries = computed(() => [
  { name: t('common.running'), color: '#2ecc71', points: metricSamples.value.map((s) => ({ at: s.at, v: s.running })) },
  { name: t('common.total'), color: '#4f7cff', points: metricSamples.value.map((s) => ({ at: s.at, v: s.total })) },
  { name: t('common.fatal'), color: '#ff5252', points: metricSamples.value.map((s) => ({ at: s.at, v: s.fatal })) },
]);
const cpuSeries = computed(() => [
  { name: 'CPU', color: '#f5a623', points: metricSamples.value.map((s) => ({ at: s.at, v: s.cpu })) },
]);
const memSeries = computed(() => [
  { name: t('serverDetail.memory'), color: '#7c5cff', points: metricSamples.value.map((s) => ({ at: s.at, v: s.mem })) },
]);
const hasResourceTrend = computed(() => metricSamples.value.some((s) => s.cpu != null || s.mem != null));
const loadSeries = computed(() => [
  { name: 'load', color: '#3aa0ff', points: metricSamples.value.map((s) => ({ at: s.at, v: s.load })) },
]);
const hostUsageSeries = computed(() => [
  { name: t('serverDetail.hostMem'), color: '#7c5cff', points: metricSamples.value.map((s) => ({ at: s.at, v: s.memPct })) },
  { name: t('serverDetail.hostDisk'), color: '#f5a623', points: metricSamples.value.map((s) => ({ at: s.at, v: s.diskPct })) },
]);
const hasHostTrend = computed(() => metricSamples.value.some((s) => s.load != null || s.memPct != null || s.diskPct != null));
...
async function daemonReload() {
  try {
    ...
    snackbar.value = { show: true, color: 'success', text: t('serverDetail.reloadDone', { a: r.added?.length || 0, c: r.changed?.length || 0, r: r.removed?.length || 0 }) };
    loadDaemon();
  } catch (e) {
...
async function sendSignalAll() {
  ...
  try {
    ...
    snackbar.value = { show: true, color: 'success', text: t('serverDetail.signalSent', { sig: signalDialog.value.signal }) };
...
<template>
  <PageShell back="/servers">
    <template #hero-title>
      ...
      <v-chip size="x-small" variant="flat" color="white" label class="text-primary">{{ methodLabel(server?.method) }}</v-chip>
    ...
    <template #hero-subtitle>
      <span class="d-flex align-center ga-3 flex-wrap">
        ...
        <span v-if="daemon && daemon.version" class="d-flex align-center ga-1">
          <v-icon size="13" icon="mdi-cog-outline" /> {{ t('serverDetail.daemonVersion', { version: daemon.version, pid: daemon.pid || '—' }) }}
        </span>
    ...
    <template #hero-actions>
      <v-menu location="bottom end" :close-on-content-click="true">
        ...
        <v-list width="320" density="comfortable" lines="two">
          ...
          <v-list-item prepend-icon="mdi-play" :title="t('serverDetail.startAll')" :subtitle="t('serverDetail.startAllDesc')" base-color="success" @click="bulk('startAll', t('serverDetail.startedAll'))" />
          <v-list-item prepend-icon="mdi-restart" :title="t('serverDetail.restartAll')" :subtitle="t('serverDetail.restartAllDesc')" base-color="info" @click="bulk('restartAll', t('serverDetail.restartedAll'))" />
          <v-list-item prepend-icon="mdi-stop" :title="t('serverDetail.stopAll')" :subtitle="t('serverDetail.stopAllDesc')" base-color="error" @click="bulk('stopAll', t('serverDetail.stoppedAll'))" />
          <v-list-item prepend-icon="mdi-signal-variant" :title="t('serverDetail.signalAll')" :subtitle="t('serverDetail.signalAllDesc')" @click="openSignalAll" />
          <v-list-item prepend-icon="mdi-broom" :title="t('serverDetail.clearAllLogs')" :subtitle="t('serverDetail.clearAllLogsDesc')" @click="bulk('clearAllLogs', t('serverDetail.clearedAllLogs'))" />
          ...
          <v-list-item
            ...
            @click="togglePause"
          />
          <v-list-item prepend-icon="mdi-medical-bag" :title="t('serverDetail.diagnose')" :subtitle="t('serverDetail.diagnoseDesc')" @click="installPanel = true" />
          <v-list-item v-if="auth.can('config:write')" prepend-icon="mdi-lightning-bolt" :title="t('events.title')" :subtitle="t('events.menuDesc')" @click="eventPanel = true" />
          <v-list-item v-if="auth.can('process:control')" prepend-icon="mdi-heart-pulse" :title="t('health.title')" :subtitle="t('health.menuDesc')" @click="healthPanel = true" />

          <v-divider class="my-1" />
          <v-list-subheader>{{ t('serverDetail.sectionDaemon') }}</v-list-subheader>
          <v-list-item prepend-icon="mdi-file-cog-outline" :title="t('config.open')" :subtitle="t('serverDetail.configDesc')" @click="configPanel = true" />
          <v-list-item prepend-icon="mdi-text-box-outline" :title="t('log.viewDaemonLog')" :subtitle="t('serverDetail.daemonLogDesc')" @click="daemonLogPanel = true" />
          <v-list-item prepend-icon="mdi-reload" :title="t('serverDetail.reload')" :subtitle="t('serverDetail.reloadDesc')" @click="daemonReload" />
          <v-list-item prepend-icon="mdi-restart-alert" :title="t('serverDetail.restartDaemon')" :subtitle="t('serverDetail.restartDaemonDesc')" @click="daemonRestart" />
          <v-list-item prepend-icon="mdi-power" :title="t('serverDetail.shutdownDaemon')" :subtitle="t('serverDetail.shutdownDaemonDesc')" base-color="error" @click="daemonShutdown" />
        </v-list>
      </v-menu>
    </template>
    ...
    <v-alert v-if="error" type="error" variant="tonal" class="mb-4">
      {{ error }}
      <template #append>
        <v-btn size="small" variant="tonal" prepend-icon="mdi-medical-bag" @click="installPanel = true">{{ t('serverDetail.diagnose') }}</v-btn>
      </template>
    </v-alert>
    ...
    <v-card v-if="snapshot" rounded="lg" class="mb-4">
      <div class="d-flex align-center px-4 py-2">
        ...
        <v-spacer />
        <v-btn-toggle v-if="showTrends" :model-value="metricRange" mandatory density="compact" variant="outlined" divided class="me-2" @update:model-value="setRange">
          <v-btn v-for="r in RANGES" :key="r.v" :value="r.v" size="small" class="px-2">{{ r.label }}</v-btn>
        </v-btn-toggle>
        <v-btn :icon="showTrends ? 'mdi-chevron-up' : 'mdi-chevron-down'" variant="text" size="small" @click="toggleTrends" />
      </div>
      <v-expand-transition>
        <div v-if="showTrends" class="px-4 pb-4">
          <v-row>
            <v-col cols="12" :md="hasResourceTrend ? 4 : 12">
              <div class="text-caption text-medium-emphasis mb-1">{{ t('serverDetail.trendProcesses') }}</div>
              <TrendChart :series="procSeries" :height="130" />
            </v-col>
            <template v-if="hasResourceTrend">
              <v-col cols="12" md="4">
                <div class="text-caption text-medium-emphasis mb-1">{{ t('serverDetail.trendCpu') }}</div>
                <TrendChart :series="cpuSeries" :height="130" unit="%" />
              </v-col>
              <v-col cols="12" md="4">
                <div class="text-caption text-medium-emphasis mb-1">{{ t('serverDetail.trendMemory') }}</div>
          ...
          <v-row v-if="hasHostTrend" class="mt-1">
            <v-col cols="12" md="6">
              <div class="text-caption text-medium-emphasis mb-1">{{ t('serverDetail.trendHostLoad') }}</div>
              <TrendChart :series="loadSeries" :height="120" />
            </v-col>
            <v-col cols="12" md="6">
              <div class="text-caption text-medium-emphasis mb-1">{{ t('serverDetail.trendHostUsage') }}</div>
              <TrendChart :series="hostUsageSeries" :height="120" unit="%" :y-max="100" />
            </v-col>
          </v-row>
          <div v-if="!hasResourceTrend && !hasHostTrend" class="text-caption text-medium-emphasis mt-2">
            <v-icon icon="mdi-information-outline" size="13" /> {{ t('serverDetail.trendNoResource') }}
          </div>
        </div>
      </v-expand-transition>
    </v-card>
    ...
    <HealthChecksPanel v-model="healthPanel" :server-id="serverId" :processes="snapshot?.processes || []" @action="onAction" />
    ...
    <v-dialog v-model="signalDialog.show" max-width="420">
      <v-card rounded="lg">
        ...
        <v-card-text>
          ...
          <v-select v-model="signalDialog.signal" :items="SIGNALS" label="Signal" variant="outlined" density="comfortable" prepend-inner-icon="mdi-signal-variant" hide-details>
            <template #selection="{ item }">SIG{{ item.value }}</template>
            <template #item="{ item, props: p }"><v-list-item v-bind="p" :title="`SIG${item.value}`" /></template>
          </v-select>
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn variant="text" @click="signalDialog.show = false">{{ t('common.cancel') }}</v-btn>
          <v-btn color="primary" variant="flat" :loading="signalDialog.busy" @click="sendSignalAll">{{ t('serverDetail.send') }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="confirmDialog.show" max-width="460">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="error" variant="flat" @click="runConfirm">{{ t('serverDetail.confirm') }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
.live-dot.off { background: rgb(var(--v-theme-error)); }
@keyframes pulse {

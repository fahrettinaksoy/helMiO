  ...
  <v-card rounded="lg" hover class="h-100 d-flex flex-column">
    ...
    <div class="d-flex align-center pa-3 pb-2">
      ...
      <div class="min-w-0 flex-grow-1">
        <router-link :to="`/servers/${vm.id}`" class="srv-name text-body-2 font-weight-bold text-truncate d-block">{{ vm.name }}</router-link>
        <div class="text-caption text-medium-emphasis text-truncate"><v-icon :icon="methodIcon(vm.method)" size="12" /> {{ methodLabel(vm.method) }}</div>
      </div>
    ...
    <div class="px-3 flex-grow-1">
      <div class="mono text-caption text-medium-emphasis text-truncate mb-2">{{ vm.connTarget }}</div>
      ...
      <template v-else-if="vm.summary">
        <div class="d-flex rounded overflow-hidden mb-2 seg-bar" style="height: 6px; gap: 2px">
          ...
          <div v-if="!vm.segments.length" class="w-100" :style="{ background: 'rgba(var(--v-theme-on-surface), 0.08)' }" />
        </div>

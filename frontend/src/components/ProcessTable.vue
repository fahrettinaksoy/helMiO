...
const statusFilters = computed(() => {
  ...
  return [
    ...
    { key: 'running', label: t('common.running'), color: 'success', count: all.filter(matchers.running).length },
    { key: 'stopped', label: t('common.stopped'), color: 'grey', count: all.filter(matchers.stopped).length },
    { key: 'fatal', label: t('common.fatal'), color: 'error', count: all.filter(matchers.fatal).length },
    { key: 'other', label: t('common.other'), color: 'warning', count: all.filter(matchers.other).length },
  ];
});
...
const groupMeta = computed(() => {
  ...
  for (const g of props.groups) {
    ...
    for (const p of g.processes) {
      if (p.cpu !== undefined) { cpu += p.cpu; hasS = true; }
...
const COLUMN_DEFS = [
  ...
  { key: 'autorestart', titleKey: 'colAutorestart', width: 110, sortable: false, needsConfig: true },
...
const DEFAULT_VISIBLE = {
  pid: true, uptime: true, cpu: true, memMb: true, start: false, exitstatus: false, restarts: false,
  priority: false, autostart: false, autorestart: false, command: false, description: true,
};
...
watch(visibleCols, (v) => {
  try { localStorage.setItem('helmio-cols', JSON.stringify(v)); } catch { /* ignore */ }
});

// Columns available in the visibility menu (hide stats columns on non-shell connectors).
const columnOptions = computed(() =>
  COLUMN_DEFS.filter((d) => (!d.needsStats || hasStats.value) && (!d.needsConfig || hasConfig.value))
);
...
const headers = computed(() => {
  ...
  for (const def of columnOptions.value) {
    ...
    cols.push({ title: t(`processTable.${def.titleKey}`), key: def.key, width: def.width, sortable: def.sortable });
  }
  cols.push({ title: t('processTable.logs'), key: 'a_logs', width: 70, align: 'center', sortable: false });
  cols.push({ title: t('processTable.start'), key: 'a_start', width: 80, align: 'center', sortable: false });
  cols.push({ title: t('processTable.stop'), key: 'a_stop', width: 80, align: 'center', sortable: false });
  cols.push({ title: t('processTable.restart'), key: 'a_restart', width: 90, align: 'center', sortable: false });
  return cols;
});
...
const startP = (p) => run(p.fullName, () => processesApi.start(props.serverId, p.fullName), t('processTable.startedMsg', { name: p.fullName }));
const stopP = (p) => run(p.fullName, () => processesApi.stop(props.serverId, p.fullName), t('processTable.stoppedMsg', { name: p.fullName }));
const restartP = (p) => run(p.fullName, () => processesApi.restart(props.serverId, p.fullName), t('processTable.restartedMsg', { name: p.fullName }));
const startG = (group) => run(`g:${group}`, () => groupsApi.start(props.serverId, group), t('processTable.groupStartedMsg', { name: group }));
const stopG = (group) => run(`g:${group}`, () => groupsApi.stop(props.serverId, group), t('processTable.groupStoppedMsg', { name: group }));
const restartG = (group) => run(`g:${group}`, () => groupsApi.restart(props.serverId, group), t('processTable.groupRestartedMsg', { name: group }));

// Multi-select bulk actions
const selected = ref([]); // fullName[]
const procByName = computed(() => Object.fromEntries(allProcesses.value.map((p) => [p.fullName, p])));
async function bulk(action) {
  const names = [...selected.value];
  busy.value = { ...busy.value, _bulk: true };
  try {
    await Promise.all(names.map((n) => (action === 'start' ? processesApi.start : processesApi.stop)(props.serverId, n)));
    emit('action', { ok: true, message: t('processTable.bulkDone', { n: names.length, action: action === 'start' ? t('processTable.start') : t('processTable.stop') }) });
    selected.value = [];
  } catch (e) {
    emit('action', { ok: false, message: e.response?.data?.error || e.message });
  } finally {
...
watch([() => props.groups, hasStats, hasConfig, statusFilter, searchField], () => nextTick(recalcHeight));
...
<template>
  ...
  <v-card variant="flat" class="table-card">
    ...
    <div class="d-flex align-center ga-2 flex-wrap pa-3">
      <template v-if="selected.length">
        <v-chip color="primary" variant="flat" size="small" label>{{ t('processTable.selected', { n: selected.length }) }}</v-chip>
        <v-btn size="small" variant="tonal" color="success" prepend-icon="mdi-play" :loading="busy._bulk" @click="bulk('start')">{{ t('processTable.startSelected') }}</v-btn>
        <v-btn size="small" variant="tonal" color="error" prepend-icon="mdi-stop" :loading="busy._bulk" @click="bulk('stop')">{{ t('processTable.stopSelected') }}</v-btn>
        <v-btn size="small" variant="text" @click="selected = []">{{ t('processTable.clearSelection') }}</v-btn>
      </template>
      <template v-else>
        <v-chip
        ...
        <template #activator="{ props: menu }">
          <v-tooltip :text="t('processTable.columns')" location="top">
            <template #activator="{ props: tip }">
              <v-btn v-bind="{ ...menu, ...tip }" icon="mdi-table-cog" size="small" variant="text" />
            </template>
          </v-tooltip>
        </template>
      ...
      <v-menu>
        <template #activator="{ props: menu }">
          <v-tooltip :text="t('processTable.export')" location="top">
            <template #activator="{ props: tip }">
              <v-btn v-bind="{ ...menu, ...tip }" icon="mdi-download" size="small" variant="text" />
            </template>
          </v-tooltip>
        </template>
        <v-list density="compact" nav>
          <v-list-item prepend-icon="mdi-file-delimited-outline" :title="t('processTable.exportCsv')" @click="exportCsv" />
          <v-list-item prepend-icon="mdi-code-json" :title="t('processTable.exportJson')" @click="exportJson" />
        </v-list>
      </v-menu>
      <v-tooltip :text="allCollapsed ? t('processTable.expandAll') : t('processTable.collapseAll')" location="top">
        <template #activator="{ props: tip }">
          <v-btn
            v-bind="tip"
    ...
    >
        ...
        <tr class="group-header-row">
          ...
          <td class="text-center">
            <v-tooltip :text="t('processTable.startGroup')" location="top">
              <template #activator="{ props: tip }">
                <v-btn v-bind="tip" icon="mdi-play" size="small" variant="text" color="success"
                  :disabled="groupMeta[item.value] && groupMeta[item.value].running >= groupMeta[item.value].total"
                  :loading="busy[`g:${item.value}`]" @click="startG(item.value)" />
              </template>
            </v-tooltip>
          </td>
          <td class="text-center">
            <v-tooltip :text="t('processTable.stopGroup')" location="top">
              <template #activator="{ props: tip }">
                <v-btn v-bind="tip" icon="mdi-stop" size="small" variant="text" color="error"
                  :disabled="groupMeta[item.value] && groupMeta[item.value].running === 0"
                  :loading="busy[`g:${item.value}`]" @click="stopG(item.value)" />
              </template>
            </v-tooltip>
          </td>
          <td class="text-center">
            <v-tooltip :text="t('processTable.restartGroup')" location="top">
              <template #activator="{ props: tip }">
                <v-btn v-bind="tip" icon="mdi-restart" size="small" variant="text" color="info"
                  :disabled="groupMeta[item.value] && groupMeta[item.value].running === 0"
                  :loading="busy[`g:${item.value}`]" @click="restartG(item.value)" />
              </template>
            </v-tooltip>
          </td>
        </tr>
      ...
      <template #[`item.restarts`]="{ item }">
        <span class="mono" :class="{ 'text-warning font-weight-bold': item.flapping }">{{ item.restarts ?? 0 }}</span>
      </template>
      ...
      <template #[`item.autorestart`]="{ item }">
        <template v-if="item.config">
          <v-chip v-if="typeof item.config.autorestart === 'string'" size="x-small" variant="tonal" label>{{ item.config.autorestart }}</v-chip>
          <v-icon v-else :icon="item.config.autorestart ? 'mdi-check-circle' : 'mdi-minus-circle-outline'" :color="item.config.autorestart ? 'success' : 'grey'" size="18" />
        </template>
        <span v-else>—</span>
      </template>

      <template #[`item.command`]="{ item }">
        <span class="mono cmd" :title="item.config?.command">{{ item.config?.command || '—' }}</span>
      </template>
      ...
      <template #[`item.a_logs`]="{ item }">
        <v-tooltip :text="t('processTable.logs')" location="top">
          <template #activator="{ props: tip }">
            <v-btn v-bind="tip" icon="mdi-text-box-outline" size="small" variant="text" @click="openLog(item)" />
          </template>
        </v-tooltip>
      </template>
      <template #[`item.a_start`]="{ item }">
        <v-tooltip :text="t('processTable.start')" location="top">
          <template #activator="{ props: tip }">
            <v-btn v-bind="tip" icon="mdi-play" size="small" variant="text" color="success"
              :disabled="isActive(item)" :loading="busy[item.fullName]" @click="startP(item)" />
          </template>
        </v-tooltip>
      </template>
      <template #[`item.a_stop`]="{ item }">
        <v-tooltip :text="t('processTable.stop')" location="top">
          <template #activator="{ props: tip }">
            <v-btn v-bind="tip" icon="mdi-stop" size="small" variant="text" color="error"
              :disabled="!isActive(item)" :loading="busy[item.fullName]" @click="stopP(item)" />
          </template>
        </v-tooltip>
      </template>
      <template #[`item.a_restart`]="{ item }">
        <v-tooltip :text="t('processTable.restart')" location="top">
          <template #activator="{ props: tip }">
            <v-btn v-bind="tip" icon="mdi-restart" size="small" variant="text" color="info"
              :disabled="!isActive(item)" :loading="busy[item.fullName]" @click="restartP(item)" />
          </template>
        </v-tooltip>
      </template>
  ...
  <LogPanel v-model="logPanel" :server-id="serverId" :process="logTarget" @action="emit('action', $event)" />
  <ProcessDetailPanel v-model="detailPanel" :server-id="serverId" :process="detailTarget" @action="emit('action', $event)" />
  </div>
</template>

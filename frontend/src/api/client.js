...
export const serversApi = {
  ...
  configFile: (id, path) => api.get(`/servers/${id}/config/file`, { params: { path } }).then((r) => r.data),
  configSave: (id, path, content) => api.put(`/servers/${id}/config/file`, { path, content }).then((r) => r.data),
  configAddProgram: (id, data) => api.post(`/servers/${id}/config/program`, data).then((r) => r.data),
  configProgramPreview: (id, data) => api.post(`/servers/${id}/config/program/preview`, data).then((r) => r.data),
  configProgramParse: (id, path) => api.get(`/servers/${id}/config/program/parse`, { params: { path } }).then((r) => r.data),
  // Event listener (push-based events)
  eventListener: (id) => api.get(`/servers/${id}/eventlistener`).then((r) => r.data),
  eventListenerInstall: (id) => api.post(`/servers/${id}/eventlistener/install`).then((r) => r.data),
  eventListenerUninstall: (id) => api.post(`/servers/${id}/eventlistener/uninstall`).then((r) => r.data),
  eventListenerRotateToken: (id) => api.post(`/servers/${id}/eventlistener/rotate-token`).then((r) => r.data),
  events: (id, limit = 200) => api.get(`/servers/${id}/events`, { params: { limit } }).then((r) => r.data),
  metrics: (id, range = 60) => api.get(`/servers/${id}/metrics`, { params: { range } }).then((r) => r.data),
  ...
  healthCheckCreate: (id, data) => api.post(`/servers/${id}/healthchecks`, data).then((r) => r.data),
  healthCheckUpdate: (id, hid, data) => api.put(`/servers/${id}/healthchecks/${hid}`, data).then((r) => r.data),
  healthCheckRemove: (id, hid) => api.delete(`/servers/${id}/healthchecks/${hid}`).then((r) => r.data),
  healthCheckRun: (id, hid) => api.post(`/servers/${id}/healthchecks/${hid}/run`).then((r) => r.data),
};

export const processesApi = {
  start: (sid, fullName) => api.post(`/servers/${sid}/processes/${encodeURIComponent(fullName)}/start`).then((r) => r.data),
  stop: (sid, fullName) => api.post(`/servers/${sid}/processes/${encodeURIComponent(fullName)}/stop`).then((r) => r.data),
  restart: (sid, fullName) => api.post(`/servers/${sid}/processes/${encodeURIComponent(fullName)}/restart`).then((r) => r.data),
  clearLog: (sid, fullName) => api.post(`/servers/${sid}/processes/${encodeURIComponent(fullName)}/log/clear`).then((r) => r.data),
  signal: (sid, fullName, signal) => api.post(`/servers/${sid}/processes/${encodeURIComponent(fullName)}/signal`, { signal }).then((r) => r.data),
  sendStdin: (sid, fullName, chars) => api.post(`/servers/${sid}/processes/${encodeURIComponent(fullName)}/stdin`, { chars }).then((r) => r.data),
  readLog: (sid, fullName, { channel = 'stdout', offset = 0, length = 32768 } = {}) =>
    api.get(`/servers/${sid}/processes/${encodeURIComponent(fullName)}/log/read`, { params: { channel, offset, length } }).then((r) => r.data),
  downloadLog: (sid, fullName, channel = 'stdout') =>
    api.get(`/servers/${sid}/processes/${encodeURIComponent(fullName)}/log/download`, { params: { channel }, responseType: 'blob' }).then((r) => r.data),
};

export const groupsApi = {
  start: (sid, group) => api.post(`/servers/${sid}/groups/${encodeURIComponent(group)}/start`).then((r) => r.data),
  stop: (sid, group) => api.post(`/servers/${sid}/groups/${encodeURIComponent(group)}/stop`).then((r) => r.data),
  restart: (sid, group) => api.post(`/servers/${sid}/groups/${encodeURIComponent(group)}/restart`).then((r) => r.data),
  signal: (sid, group, signal) => api.post(`/servers/${sid}/groups/${encodeURIComponent(group)}/signal`, { signal }).then((r) => r.data),
};
...
export const bulkApi = {
  ...
  signalAll: (sid, signal) => api.post(`/servers/${sid}/bulk/signal-all`, { signal }).then((r) => r.data),

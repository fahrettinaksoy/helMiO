...
// Global FATAL / flapping bildirim snackbar'ı (realtime store'dan beslenir).
...
const nav = computed(() => {
  ...
  // Tek-kullanıcı masaüstü: bildirim kanalları + denetim günlüğü kalır;
  // kullanıcı yönetimi ve API token'ları kaldırıldı.
...
const socials = [
  {
    name: 'X / Twitter',
    icon: 'mdi-twitter',
    url: `https://twitter.com/intent/tweet?text=${enc(SHARE_TEXT)}&url=${enc(REPO_URL)}`
  },
  {
    name: 'LinkedIn',
    icon: 'mdi-linkedin',
    url: `https://www.linkedin.com/sharing/share-offsite/?url=${enc(REPO_URL)}`
  },
  {
    name: 'Reddit',
    icon: 'mdi-reddit',
    url: `https://www.reddit.com/submit?url=${enc(REPO_URL)}&title=${enc(SHARE_TEXT)}`
  },
  {
    name: 'Facebook',
    icon: 'mdi-facebook',
    url: `https://www.facebook.com/sharer/sharer.php?u=${enc(REPO_URL)}`
  },
  {
    name: 'WhatsApp',
    icon: 'mdi-whatsapp',
    url: `https://wa.me/?text=${enc(`${SHARE_TEXT} ${REPO_URL}`)}`
  },
  {
    name: 'Telegram',
    icon: 'mdi-send',
    url: `https://t.me/share/url?url=${enc(REPO_URL)}&text=${enc(SHARE_TEXT)}`
  }
]
  ...
  <v-app>
    ...
    <template v-else>
      ...
      <v-app-bar flat elevation="3" color="surface" density="comfortable">
        ...
        <span class="text-body-2 text-medium-emphasis d-none d-sm-flex">{{
          t('app.subtitle')
        }}</span>
          ...
          <v-list density="compact" nav>
            <v-list-item>
              ...
              <v-list-item-subtitle>
                <v-chip :color="roleColor(auth.role)" size="x-small" variant="tonal" label>{{
                  t(`roles.${auth.role}`)
                }}</v-chip>
              </v-list-item-subtitle>
            </v-list-item>
      ...
      >
        <v-list nav density="comfortable" class="px-2 py-4">
          <div v-if="!rail" class="text-overline text-medium-emphasis px-3 mb-1">
            {{ t('app.menu') }}
          </div>
          <v-tooltip
            v-for="item in nav"
        ...
        <template #append>
          ...
          <div v-if="rail" class="pa-2 pb-3 d-flex justify-center">
            <v-tooltip
              :text="realtime.connected ? t('app.liveActive') : t('app.noConnection')"
              location="end"
            >
              <template #activator="{ props: tip }">
                <v-icon
      ...
      <ServerFormDrawer
        v-model="ui.serverFormOpen"
        :server="ui.serverFormTarget"
        @saved="serversStore.fetchAll()"
      />

      <v-snackbar
        v-model="alertSnack.show"
...
@keyframes pulse {
  0% {
    box-shadow: 0 0 0 0 rgba(var(--v-theme-success), 0.45);
  }
  70% {

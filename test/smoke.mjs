...
async function main() {
  ...
  console.log('\n== auth + rbac ==');
  const setup = await fetch(`${B}/api/auth/setup`, {
    method: 'POST',
  ...
  const vu = await fetch(`${B}/api/users`, {
    method: 'POST',
    headers: A,
    body: JSON.stringify({ username: 'viewer1', password: 'viewerpass1', role: 'viewer' }),
  });
  ...
  const vtok = (
    await fetch(`${B}/api/auth/login`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ username: 'viewer1', password: 'viewerpass1' }),
    }).then(J)
  ).token;
  const vcreate = await fetch(`${B}/api/servers`, {
    method: 'POST',
    headers: auth(vtok),
    body: JSON.stringify({ method: 'tcp', name: 'x', host: '127.0.0.1', port: 9001 }),
  });
  ok('viewer cannot create server (403)', vcreate.status === 403, `got ${vcreate.status}`);

  console.log('\n== secret encryption at rest ==');
  const srv = await fetch(`${B}/api/servers`, {
    method: 'POST',
    headers: A,
    body: JSON.stringify({
      method: 'tcp',
  ...
  console.log('\n== api tokens ==');
  const tk = await fetch(`${B}/api/apitokens`, {
    method: 'POST',
    headers: A,
    body: JSON.stringify({ name: 'ci', role: 'operator' }),
  }).then(J);
  ok(
    'token created (hmo_ plaintext once)',
    tk.token.startsWith('hmo_') && tk.record.prefix.startsWith('hmo_'),
  );
  ...
  const tokenCreate = await fetch(`${B}/api/servers`, {
    method: 'POST',
    headers: auth(tk.token),
    body: JSON.stringify({ method: 'tcp', name: 'y', host: '127.0.0.1', port: 9001 }),
  });
  ok(
    'operator token cannot create server (403)',
    tokenCreate.status === 403,
    `got ${tokenCreate.status}`,
  );

  console.log('\n== notification channel ==');
  const ch = await fetch(`${B}/api/channels`, {
    method: 'POST',
    headers: A,
    body: JSON.stringify({
      type: 'webhook',
      name: 'hook',
      config: { url: 'http://127.0.0.1:4700/hook' },
  ...
  ok(
    'channel test delivered to webhook',
    !!lastWebhook && /test/.test(lastWebhook.text || lastWebhook.message || ''),
  );

  console.log('\n== event ingest + alert routing ==');
  const tok2 = (
    await fetch(`${B}/api/servers/${srv.id}/eventlistener/rotate-token`, {
      method: 'POST',
      headers: A,
    }).then(J)
  ).token;
  lastWebhook = null;
  await fetch(`${B}/api/ingest/${srv.id}/events`, {
    method: 'POST',
    headers: { 'content-type': 'application/json', authorization: `Bearer ${tok2}` },
    body: JSON.stringify({
      eventname: 'PROCESS_STATE_FATAL',
      payload: { processname: 'w', groupname: 'w' },
    }),
  });
  ...
  const prev = await fetch(`${B}/api/servers/${srv.id}/config/program/preview`, {
    method: 'POST',
    headers: A,
  ...
  console.log('\n== health checks ==');
  const hc = await fetch(`${B}/api/servers/${srv.id}/healthchecks`, {
    method: 'POST',
    headers: A,
    body: JSON.stringify({
      target: 'w:w',
      ...
      failureThreshold: 2,
      action: 'alert',
      config: { url: 'http://127.0.0.1:4700/health', expectStatus: 200, timeoutMs: 1000 },
    }),
  }).then(J);
  ok('health check created', !!hc.id);
  healthy = true;
  const run1 = await fetch(`${B}/api/servers/${srv.id}/healthchecks/${hc.id}/run`, {
    method: 'POST',
    headers: A,
  }).then(J);
  ok('http probe ok when healthy', run1.ok === true);
  healthy = false;
  const run2 = await fetch(`${B}/api/servers/${srv.id}/healthchecks/${hc.id}/run`, {
    method: 'POST',
    headers: A,
  }).then(J);
  ...
  console.log('\n== fleet validation ==');
  const fbad = await fetch(`${B}/api/fleet/run`, {
    method: 'POST',
    headers: A,
    body: JSON.stringify({ action: 'restartAll', serverIds: [] }),
  });
